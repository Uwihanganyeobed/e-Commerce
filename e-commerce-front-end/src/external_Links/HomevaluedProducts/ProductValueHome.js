...
import "./HomeproductValue.css";

function HomeProductValue() {
  // Filter products to get only those with the category 'Home'
  const Product_Home = all_products.filter(product => product.prodCategory === 'Equip-Home');

  // Group products in sets of three
  const productsGrouped = [];
  for (let i = 0; i < Product_Home.length; i += 3) {
    productsGrouped.push(Product_Home.slice(i, i + 3));
  }
              ...
              <div className="product-deals">
                {group.map((item, itemIndex) => (
                  <div key={itemIndex} className="product-container">
                    ...
                    <Link to={`/products/home/${item.prodId}`}>
...
export default HomeProductValue;
